...
export default function CharacterPicker({ onSelect, onBack }) {
  ...
  return (
    <div className="flex flex-col gap-5 animate-pop w-full">
      {/* Header — icon + max 2 words */}
      <div className="text-center">
        <p className="text-5xl mb-2">📖</p>
        <p className="text-3xl font-black text-purple-800">מי מספר?</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {CHARACTERS.map(ch => (
          <button
            ...
            className={`
              relative rounded-3xl py-6 px-3 flex flex-col items-center gap-2
          ...
          >
            ...
            <span className="font-black text-gray-700 text-xl">{ch.label}</span>
          </button>
        ))}
      </div>
